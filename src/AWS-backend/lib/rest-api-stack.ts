...
export class RestAPIStack extends cdk.Stack {
  ...
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    // Tables
    const usersTable = new dynamodb.Table(this, "UserTables", {
      billingMode: dynamodb.BillingMode.PAY_PER_REQUEST,
    ...
    const newUserFn = new lambdanode.NodejsFunction(this, "NewUserFn", {
      architecture: lambda.Architecture.ARM_64,
      runtime: lambda.Runtime.NODEJS_16_X,
      entry: `${__dirname}/../lambdas/users/addUser.ts`,
      timeout: cdk.Duration.seconds(10),
      memorySize: 128,
      environment: {
    ...
    const newBookingFn = new lambdanode.NodejsFunction(
      this,
      "newBookingFn",
      {
        architecture: lambda.Architecture.ARM_64,
        runtime: lambda.Runtime.NODEJS_16_X,
        entry: `${__dirname}/../lambdas/bookings/addBooking.ts`,
        timeout: cdk.Duration.seconds(10),
        memorySize: 128,
        environment: {
    ...
    const getUserById = new lambdanode.NodejsFunction(
      this,
      "GetUserById",
      {
        architecture: lambda.Architecture.ARM_64,
        runtime: lambda.Runtime.NODEJS_16_X,
        entry: `${__dirname}/../lambdas/users/getUserById.ts`,
        timeout: cdk.Duration.seconds(10),
        memorySize: 128,
        environment: {
          TABLE_NAME: usersTable.tableName,
    ...
    const getBookingById = new lambdanode.NodejsFunction(
      this,
      "GetBookingById",
      {
        architecture: lambda.Architecture.ARM_64,
        runtime: lambda.Runtime.NODEJS_16_X,
        entry: `${__dirname}/../lambdas/bookings/getBookingByUserId.ts`,
        timeout: cdk.Duration.seconds(10),
        memorySize: 128,
        environment: {
          TABLE_NAME: bookingsTable.tableName,
    ...
    const updateUser = new lambdanode.NodejsFunction(
      this,
      "UpdateUser",
      {
        architecture: lambda.Architecture.ARM_64,
        runtime: lambda.Runtime.NODEJS_16_X,
        entry: `${__dirname}/../lambdas/users/updateUser.ts`,
        timeout: cdk.Duration.seconds(10),
        memorySize: 128,
        environment: {
          TABLE_NAME: usersTable.tableName,
    ...
    accommodationPageTable.grantReadWriteData(getAccomadationPageById)
    ...
    usersTable.grantReadWriteData(newUserFn)
    usersTable.grantReadWriteData(getUserById)
    usersTable.grantReadWriteData(updateUser)

    bookingsTable.grantWriteData(newBookingFn)
    bookingsTable.grantReadData(getBookingById)
    ...
    const accomadationPageIdEndpoint = accomadationPageEndPoint.addResource("{accId}");
    accomadationPageIdEndpoint.addMethod("GET", new apig.LambdaIntegration(getAccomadationPageById))
    ...
    userEndPoint.addMethod("POST", new apig.LambdaIntegration(newUserFn, {proxy: true}));
    const userIdEndPoint = userEndPoint.addResource("{userId}");
    userIdEndPoint.addMethod("GET", new apig.LambdaIntegration(getUserById, {proxy: true}));
    userIdEndPoint.addMethod("PUT", new apig.LambdaIntegration(updateUser, {proxy: true}));


    const bookingEndPoint = api.root.addResource("booking");
    bookingEndPoint.addMethod("POST", new apig.LambdaIntegration(newBookingFn, {proxy: true}));
    bookingEndPoint.addMethod("GET", new apig.LambdaIntegration(getBookingById, {
      proxy: true}))}
}
