...
export const handler: APIGatewayProxyHandlerV2 = async (event, context) => {
    try {
        ...
        // Check if the year parameter is present in the queryParams
        if (year) {

            commandInput.KeyConditionExpression += " and begins_with(reviewDate, :y)";
            // @ts-ignore
            commandInput.ExpressionAttributeValues[":y"] = year;
        }
